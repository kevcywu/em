...
public class WZTool {
    ...
    public static String readDecodedString(LittleEndianAccessor llea, boolean modernImg) {
        int strLength;
        ...
        if (b >= 0) {
            ...
            if (modernImg) {
                str = xorCharArray(str, MODERN_UNI_KEY);
            }
            ...
            if (modernImg) {
                str = xorCharArray(str, MODERN_KEY);
            }
            return String.valueOf(str);
        }
    }

    public static String readDecodedStringAtOffset(SeekableLittleEndianAccessor slea, int offset, boolean modernImg) {
        slea.seek(offset);
        return readDecodedString(slea, modernImg);
    }

    public static String readDecodedStringAtOffsetAndReset(SeekableLittleEndianAccessor slea, int offset, boolean modernImg) {
        long pos = 0;
        pos = slea.getPosition();
        slea.seek(offset);
        String ret = readDecodedString(slea, modernImg);
        slea.seek(pos);
        return ret;
    }
