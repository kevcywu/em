...
import net.sf.world.remote.WorldChannelInterface;
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (mode == 1) { // add
			...
			if (ble != null && !ble.isVisible()) {
				c.getSession().write(MaplePacketCreator.serverNotice(1, "You already have \"" + ble.getName() +
					"\" on your Buddylist"));
				...
				try {
					...
					if (charWithId != null) {
						...
						if (buddyAddResult == BuddyAddResult.BUDDYLIST_FULL) {
							c.getSession().write(MaplePacketCreator.serverNotice(1, "\"" + addName +
								"\"'s Buddylist is full"));
						} else {
						...
						c.getSession().write(MaplePacketCreator.serverNotice(1, "A character called \"" + addName +
							"\" does not exist"));
					}
